<?php
session_start();
$title = "Agregar Interior";
/* Llamar la Cadena de Conexion*/
include("../conexion.php");
//Insert un nuevo producto
$imagen_demo = "demo.png";
$insert = mysqli_query($con, "insert into interior (url_image, estado) values ('$imagen_demo','0')");
$sql_last = mysqli_query($con, "select LAST_INSERT_ID(id) as last from interior order by id desc limit 0,1");
$rw = mysqli_fetch_array($sql_last);
$id_interior = intval($rw['last']);
$sql = mysqli_query($con, "select * from interior where id='$id_interior' limit 0,1");
...
<div class="container add">
	...
	<div class="row">
		...
		<ol class="breadcrumb">
			...
			<li><a href="interiorlist.php">Interiores</a></li>
			<li class="active">Agregar</li>
		</ol>
		<div class="col-md-7">
			<h3><span class="glyphicon glyphicon-edit"></span>Agregar interior</h3>
			<form class="form-horizontal" id="editar_interior">
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						...
						<input type="hidden" class="form-control" id="id_interior" value="<?php echo intval($id_interior); ?>" name="id_interior">
					</div>
				</div>
...
<script>
	function upload_image() {
		...
		var id_interior = $("#id_interior").val();
		var inputFileImage = document.getElementById("fileToUpload");
		var file = inputFileImage.files[0];
		var data = new FormData();
		data.append('fileToUpload', file);
		data.append('id', id_interior);

		$.ajax({
			url: "ajax/upload_interior.php", // Url to which the request is send
...
<script>
	$("#editar_interior").submit(function(e) {

		$.ajax({
			url: "ajax/editar_interior.php",
			type: "POST",
			data: $("#editar_interior").serialize(),
			beforeSend: function(objeto) {
