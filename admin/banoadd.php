...
$insert = mysqli_query($con, "insert into bano (url_image, estado) values ('$imagen_demo','0')");
$sql_last = mysqli_query($con, "select LAST_INSERT_ID(id) as last from bano order by id desc limit 0,1");
$rw = mysqli_fetch_array($sql_last);
$id_bano = intval($rw['last']);
$sql = mysqli_query($con, "select * from bano where id='$id_bano' limit 0,1");
	...
	<div class="container">
		...
		<div class="row">
			...
			<ol class="breadcrumb">
				...
				<li><a href="banolist.php">Baños</a></li>
				<li class="active">Agregar</li>
			</ol>
			<div class="col-md-7">
				<h3><span class="glyphicon glyphicon-edit"></span> Agregar baño</h3>
				<form class="form-horizontal" id="editar_bano">
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							...
							<input type="hidden" class="form-control" id="id_bano" value="<?php echo intval($id_bano); ?>" name="id_bano">
						</div>
					</div>
...
<script>
	function upload_image() {
		...
		var id_bano = $("#id_bano").val();
		var inputFileImage = document.getElementById("fileToUpload");
		var file = inputFileImage.files[0];
		var data = new FormData();
		data.append('fileToUpload', file);
		data.append('id', id_bano);

		$.ajax({
			url: "ajax/upload_bano.php", // Url to which the request is send
...
<script>
	$("#editar_bano").submit(function(e) {

		$.ajax({
			url: "ajax/editar_bano.php",
			type: "POST",
			data: $("#editar_bano").serialize(),
			beforeSend: function(objeto) {
